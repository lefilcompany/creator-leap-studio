...
export default defineTool({
  ...
  description: "Lista as notificações do usuário autenticado, com filtro por lida/não lida.",
  inputSchema: {
  ...
  handler: async ({ only_unread, limit, offset }, ctx) => {
    ...
    return ok({ items: data ?? [], total: count ?? 0, limit, offset }, `${data?.length ?? 0} notificação(ões).`);
  },
});
