...
import { ChevronDown, ChevronUp, Coins, ShieldAlert } from "lucide-react";
    ...
    <article className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {tool.costCredits ? (
            <Badge variant="secondary" className="gap-1">
              <Coins className="h-3 w-3" /> {tool.costCredits} crédito{tool.costCredits > 1 ? "s" : ""}
            </Badge>
          ) : null}
          {tool.destructive ? (
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="space-y-2">
          ...
          <CodeBlock code={JSON.stringify(tool.exampleRequest, null, 2)} language="request" />
        </div>
        <div className="space-y-2">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
            Exemplo de response
          </h2>
          <CodeBlock code={JSON.stringify(tool.exampleResponse, null, 2)} language="response" />
        </div>
      </section>
