...
serve(async (req) => {
  ...
  try {
    ...
    /* OLD HTML TEMPLATE - Removido em favor do template TypeScript
    const htmlTemplate = `
<!DOCTYPE html>
<html lang="pt-BR">
<head>
  ...
  <style>
    ...
    .container {
      ...
      background: #fff;
      border-radius: 16px;
      box-shadow: 0 8px 32px rgba(194, 22, 96, 0.10);
      overflow: hidden;
      border: 1.5px solid rgba(194, 22, 96, 0.10);
    }
    .header {
      background: linear-gradient(90deg, hsl(330,100%,38%) 0%, hsl(269,66%,48%) 100%);
      color: #fff;
    ...
    .logo {
      max-width: 140px;
      margin-bottom: 18px;
      filter: brightness(0) invert(1) drop-shadow(0 2px 8px rgba(255,255,255,0.10));
    }
    .header h1 {
      margin: 0;
      font-size: 1.7rem;
...
<body>
  <div class="container">
    <div class="header">
      <img src="https://afxwqkrneraatgovhpkb.supabase.co/storage/v1/object/public/content-images/logoCreatorPreta.png" alt="Logo Creator IA" class="logo">
      <h1>Recuperação de Senha</h1>
    </div>
    <div class="content">
      <h2>Olá!</h2>
      ...
      <p>Para criar uma nova senha, clique no botão abaixo. Este link é seguro e exclusivo para você.</p>
      <div class="button-container">
        <a href="${confirmationUrl}" class="reset-button">Redefinir Minha Senha</a>
      </div>
      <div class="warning">
        <strong>⚠️ Atenção:</strong> Este link expirará em breve e só pode ser usado uma vez por motivos de segurança.
      </div>
      <p>Se o botão não funcionar, copie e cole o seguinte link no seu navegador:</p>
      <div class="url-fallback">${confirmationUrl}</div>
      <p style="margin-top: 24px; font-size: 0.95rem;">Caso você não tenha solicitado esta redefinição, pode ignorar este e-mail com segurança. Sua senha permanecerá inalterada.</p>
    </div>
    <div class="footer">
      <p><strong>Creator IA</strong></p>
