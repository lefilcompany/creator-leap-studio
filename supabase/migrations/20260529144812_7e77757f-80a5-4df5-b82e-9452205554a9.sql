...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      NULLIF(a.result->>'title', ''),
      NULLIF(a.result->'carousel'->'caption'->>'title', ''),
      NULLIF(a.result->>'headline', ''),
      NULLIF(split_part(a.result->>'legenda', E'\n', 1), ''),
      NULLIF(a.details->>'description', ''),
      CASE
        WHEN a.result->>'description' IS NOT NULL
         AND a.result->>'description' <> 'Imagem gerada com sucesso'
        THEN a.result->>'description'
      END
    )::text as title,
