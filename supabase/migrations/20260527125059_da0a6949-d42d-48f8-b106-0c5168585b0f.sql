...
BEGIN
  ...
  -- 1. Detach from the team on the profile (loses RLS access to team resources)
  UPDATE public.profiles
  SET team_id = NULL, updated_at = NOW()
  WHERE id = p_member_id AND team_id = p_team_id;

  -- 2. Remove from team_members join table
  DELETE FROM public.team_members
  WHERE user_id = p_member_id AND team_id = p_team_id;

  -- 3. Clean up old join requests so the user can request to join again later
