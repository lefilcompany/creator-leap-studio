-- Criar função segura para criar equipe (SECURITY DEFINER bypassa RLS)
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = p_user_id AND team_id IS NOT NULL) THEN
