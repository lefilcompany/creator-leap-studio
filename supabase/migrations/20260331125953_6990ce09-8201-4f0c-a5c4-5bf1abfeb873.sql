...
-- Add composite indexes for faster history queries
CREATE INDEX IF NOT EXISTS idx_actions_user_deleted_created 
  ON public.actions (user_id, created_at DESC, id DESC) 
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_actions_team_deleted_created 
  ON public.actions (team_id, created_at DESC, id DESC) 
  WHERE deleted_at IS NULL;

-- Optimized get_action_summaries: removes expensive COUNT(*) scan
-- Uses estimated total from first page instead
...
RETURNS TABLE(
  id uuid, type text, created_at timestamp with time zone, approved boolean,
  brand_id uuid, brand_name text, image_url text, title text, platform text,
  objective text, total_count bigint, thumb_path text, video_url text
)
...
-- Also update the team-only overload
CREATE OR REPLACE FUNCTION public.get_action_summaries(
  p_team_id uuid,
  p_brand_filter uuid DEFAULT NULL::uuid,
  p_type_filter text DEFAULT NULL::text,
